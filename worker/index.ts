...
const STOCK_CHECK_URLS: Record<string, Record<string, string>> = {
  ...
  x4c: {
    en: 'https://api-prod.xteink.cc/api/v1/check-update?current_version=V0.0.1&device_type=ESP32S3_X4_CLA_SSD1677&device_id=12345&lng=en&ota_type=1',
  },
};
...
// --- KOReader Sync (kosync) Account Registration ---
...
// protocol stores the MD5 of the password — the reader hashes it before sending,
// so we mirror that here (Web Crypto has no MD5, hence the local implementation).
async function handleKosyncRegister(
  request: Request,
  headers: Record<string, string>
): Promise<Response> {
  if (request.method !== 'POST') {
  ...
  let body: { username?: string; password?: string; server?: string };
  try {
  ...
  const serverInput = (body.server || '').trim() || DEFAULT_KOSYNC_SERVER;

  if (!username || !password) {
    return json({ error: 'Username and password are required.' }, 400, headers);
  }
  if (username.length < 3) {
    return json({ error: 'Username must be at least 3 characters.' }, 400, headers);
  }
  if (password.length < 4) {
    return json({ error: 'Password must be at least 4 characters.' }, 400, headers);
  }

  // Only allow http(s) targets — this is an open proxy to the given host, so
  // reject anything that isn't a normal web URL to avoid pointing it at
  // internal schemes.
  let base: URL;
  try {
  ...
  // kosync appends /users/create under the server root; preserve any path prefix.
  const createUrl = new URL('users/create', base.href.endsWith('/') ? base.href : base.href + '/').toString();

  let upstream: Response;
  try {
    upstream = await fetch(createUrl, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/vnd.koreader.v1+json',
      },
      body: JSON.stringify({ username, password: md5Hex(password) }),
      signal: AbortSignal.timeout(15000),
    });
  } catch {
    return json({ error: 'Could not reach that sync server. Check the URL and try again.' }, 502, headers);
  }

  const text = await upstream.text();
  let data: { username?: string; code?: number; message?: string } = {};
  try {
    data = text ? JSON.parse(text) : {};
  } catch {
    // Non-JSON upstream body — treat as an opaque error below.
  }

  if (upstream.ok || upstream.status === 201) {
    return json({ ok: true, username: data.username || username, server: serverInput }, 200, headers);
  }

  // kosync returns 402 with code 2002 when the username already exists.
  const message =
    data.message ||
    (upstream.status === 402 ? 'That username is already registered.' : `Registration failed (HTTP ${upstream.status}).`);
  return json({ error: message }, upstream.status === 402 ? 409 : 502, headers);
}

// MD5 (RFC 1321). Web Crypto doesn't offer MD5, but the kosync protocol requires
// the password to be sent as its MD5 hex digest (the reader hashes the same way).
// This is the well-tested Paul Johnston / blueimp core, UTF-8 aware.
function md5Hex(str: string): string {
  ...
  function cycle(state: number[], k: number[]): void {
  ...
  for (let i = 0; i < n * 8; i += 8) {
    bin[i >> 5] |= (utf8.charCodeAt(i / 8) & 0xff) << (i % 32);
  }
  bin[n * 8 >> 5] |= 0x80 << ((n * 8) % 32);
  bin[(((n * 8 + 64) >>> 9) << 4) + 14] = n * 8;

  const state = [1732584193, -271733879, -1732584194, 271733878];
  for (let i = 0; i < bin.length; i += 16) {
    const block: number[] = [];
    for (let j = 0; j < 16; j++) block[j] = bin[i + j] | 0;
    cycle(state, block);
  }
