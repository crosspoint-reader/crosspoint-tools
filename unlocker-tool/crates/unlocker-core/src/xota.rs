...
//!   [36:38]   channel flags (`00 00` for ota_type 0, `01 00` for ota_type 1)
...
//! ```
//!
//! Xteink uses a different static key and metadata flag for each OTA channel.
//! The two adjacent 16-byte tables at DROM `0x3c2f0530` of the X4 Pro factory
//! image are transformed as `key[i] = table[i] ^ ((17*i - 0x5b) & 0xff)`.
//! Channel 0 uses flags `00 00`; channel 1 uses flags `01 00`.
...
const KEY_TABLES: [[u8; 16]; 2] = [
    [
        0x2a, 0x28, 0xd5, 0xe4, 0x8a, 0x2b, 0xa8, 0x58, 0x04, 0xa5, 0xbf, 0x78, 0x6c, 0x8c, 0x63,
        0x91,
    ],
    [
        0xea, 0x91, 0x56, 0x60, 0xe4, 0x2d, 0x51, 0x76, 0xc2, 0x33, 0x3d, 0xea, 0x48, 0x2a, 0x53,
        0xfa,
    ],
];

/// Firmware key derivation: `key[i] = table[i] ^ ((17*i - 0x5b) & 0xff)`.
/// The selected table is determined by the device's `ota_type` channel.
fn derive_key(ota_type: u8) -> anyhow::Result<[u8; 16]> {
    let table = KEY_TABLES
        .get(ota_type as usize)
        .ok_or_else(|| anyhow::anyhow!("unsupported X4 Pro ota_type {ota_type}"))?;
    let mut key = [0u8; 16];
...
fn build_metadata(
    ...
    m[36..38].copy_from_slice(&[ota_type, 0]);
...
pub struct EncryptedXota {
    /// OTA channel whose static key and metadata flags were used.
    pub ota_type: u8,
...
pub fn inspect(xota: &[u8]) -> anyhow::Result<XotaInfo> {
    ...
    for ota_type in 0..=1 {
        ...
        if declared_plain_size == actual_plain_size && meta[36..38] == [ota_type, 0] {
            matched = Some((ota_type, meta));
            break;
        }
    }
    let (ota_type, meta) = matched.ok_or_else(|| {
        anyhow::anyhow!(
            "invalid .xota metadata for ota_type 0 and 1 (package contains {actual_plain_size} plaintext bytes)"
        )
    })?;
    let plain_sha256 = hex::encode(&meta[4..36]);
    Ok(XotaInfo {
        ota_type,
        plain_size: actual_plain_size,
        plain_sha256,
...
mod tests {
    ...
    /// Both key tables must reproduce the keys verified against current stock
    /// ota_type_0 and ota_type_1 packages.
    #[test]
    fn key_derivation_matches_stock_channels() {
        assert_eq!(
            hex::encode(derive_key(0).unwrap()),
            "8f9e123c63d1a344299bf0181d0ef035"
        );
        assert_eq!(
            hex::encode(derive_key(1).unwrap()),
            "4f2791b80dd75a6aef0d728a39a8c05e"
        );
        assert!(derive_key(2).is_err());
    }

    #[test]
    fn inspects_stock_ota_type_zero_and_one_metadata() {
        // First 202 bytes of Xteink's V7.4.1 ota_type_0/1 packages fetched on
        // 2026-08-22. `inspect` only decrypts this metadata prefix; resizing to
        // the advertised package length supplies the plaintext-size check.
        let prefixes = [
            (0, ""),
            (1, ""),
        ];
        for (ota_type, prefix) in prefixes {
            let mut package = hex::decode(prefix).unwrap();
            package.resize(5_529_482, 0);
            let info = inspect(&package).unwrap();
            assert_eq!(info.ota_type, ota_type);
            assert_eq!(info.plain_size, 5_529_280);
            assert_eq!(
                info.plain_sha256,
                "d10754620e4aefee02293effd85178830ac3451ff2bae80c55d22bb16652aa14"
            );
        }
    }
    ...
    fn round_trips_plain_image() {
        let plain: Vec<u8> = (0..50_000u32)
            ...
            .collect();
        let out = encrypt(Model::X4Pro, &plain, "V9.9.9", 0, None).unwrap();
    ...
    fn fixed_iv_is_deterministic() {
        ...
        let a = encrypt(Model::X4Pro, &plain, "V7.0.9", 1, Some(iv)).unwrap();
        let b = encrypt(Model::X4Pro, &plain, "V7.0.9", 1, Some(iv)).unwrap();
        assert_eq!(a.bytes, b.bytes, "same IV + input ⇒ identical package");
        assert_eq!(inspect(&a.bytes).unwrap().ota_type, 1);
        assert_ne!(
            a.bytes,
            encrypt(Model::X4Pro, &plain, "V7.0.9", 0, Some(iv))
                .unwrap()
                .bytes,
            "ota_type 0 and 1 use different keys"
        );
    }
}
