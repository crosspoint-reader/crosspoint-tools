...
export default function CollaborateForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="mx-auto max-w-xl space-y-3 text-left">
      ...
      {status && (
        <p
          role="status"
          className={`rounded-lg px-4 py-3 text-sm/6 ${status.ok ? 'bg-brand-50 text-brand-700' : 'bg-red-50 text-red-800'}`}
        >
          {status.msg}
        </p>
      )}
